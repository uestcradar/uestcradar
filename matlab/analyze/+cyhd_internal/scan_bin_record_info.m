...
%     0x27080  128 bytes  frame tail (0xA5)
    ...
    pulseTransitions = emptyTransitionStats();
    ...
    while searchOffset < fileSize
        ...
        if fullyValid
            ...
            if isAdjacent
                ...
                [pulseTransitions, pulseIssue] = updateTransition(pulseTransitions, ...
                    double(previousMeta.pulse_in_beam), double(meta.pulse_in_beam));
                if ~isempty(pulseIssue)
                    addSemanticTransitionIssue(pulseIssue, 'pulse_in_beam', ...
                        previousMeta.pulse_in_beam, meta.pulse_in_beam, candidateOffset);
                end
            ...
            timestampAdjacent = segmentActive && ...
                (meta.timestamp == lastValidMeta.timestamp || ...
                (meta.timestamp > lastValidMeta.timestamp && ...
                meta.timestamp - lastValidMeta.timestamp == protocol.timestamp_ticks_per_frame));
            if physicalAdjacent && ~timestampAdjacent && ...
                    meta.timestamp >= lastValidMeta.timestamp
                addIssue('SEMANTIC', 'WARNING', 'TIMESTAMP_DISCONTINUITY', candidateCount, ...
                    uint64(candidateOffset + protocol.beam_offset + 8), uint64(8), ...
                    'timestamp', sprintf('%u', lastValidMeta.timestamp), ...
                    sprintf('%u', meta.timestamp), ...
                    sprintf('相邻有效帧时间戳增量不是正常的%d tick。', ...
                    protocol.timestamp_ticks_per_frame));
            end
            startsNewSegment = ~segmentActive || ~physicalAdjacent || ~timestampAdjacent;
            if startsNewSegment
    ...
    record_info.semantic = struct( ...
        ...
        'pulse_in_beam', pulseTransitions, ...
