...
%ANALYZE_BIN_GUI Validate and convert one continuous CYHD BIN capture.
    ...
    actionGrid.ColumnWidth = {'1x', 150, 150};
    actionGrid.Padding = [8, 4, 8, 4];
    actionGrid.ColumnSpacing = 8;

    uilabel(actionGrid, ...
        'Text', '每个通道按原始采样时间顺序保存为一维复数列向量，不进行PRI切分', ...
        'FontColor', secondaryColor);
    ...
    function runSave(~, ~)
        ...
        try
            ...
            targetPaths = getDataTargetPaths(record_info, outputDir);
            if isempty(targetPaths)
                error('CYHD:NoValidSegment', '没有可保存的连续有效段。');
            end
            ...
            progressGauge.Value = 0;
            save_summary = cyhd_internal.save_bin_data_mat(selectedBinPath, record_info, ...
                'OutputDir', outputDir, ...
                'Overwrite', true, ...
    ...
    function paths = getDataTargetPaths(info, outputDir)
        [~, stem] = fileparts(selectedBinPath);
        paths = {};
        for row = 1:height(info.segments)
            if info.segments.frame_count(row) > 0
    ...
    function printReport(info)
        ...
        emitLine(sprintf('sweep/beam语义异常: %s / %s；时间戳回退: %s', ...
    ...
    function printSaveSummary(summary, info, reportPath)
        ...
        emitLine(sprintf('总保存有效XDMA帧: %s；总连续采样点: %s', ...
            formatUint(summary.saved_frame_count), ...
            formatUint(summary.saved_sample_count)));
        emitLine(sprintf('跳过坏帧: %s；不连续点: %s', ...
            formatUint(summary.skipped_bad_frame_count), ...
            formatUint(info.summary.gap_count)));
        for row = 1:height(summary.files)
            emitLine(sprintf('  段%d: %s帧，%s个连续采样点，通道向量[%s × 1]。', ...
                summary.files.segment_id(row), ...
                formatUint(summary.files.frame_count(row)), ...
                formatUint(summary.files.sample_count(row)), ...
                formatUint(summary.files.sample_count(row))));
