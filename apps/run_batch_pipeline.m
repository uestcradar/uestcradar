...
%% 1b. 参数区：波位切换（多波位 TWS 模式）
cfg.beam.enable = true;                 % 是否启用多波位处理；false 时按原有连续流模式运行
cfg.beam.beam_file = '波位格式.txt';     % 波位排布文件路径（相对于工程根目录）
...
for di = 1:numel(cfg.paths.data_folders)
    ...
    for bi = 1:numel(raw_specs)
        ...
        % 构建基础 RD 上下文（波位和传统模式都需要 pri_len, fs, prt 等基础参数）
        ...
        if cfg.beam.enable
            ...
            for beam_id = 1:beam_schedule.num_beams
                ...
                beam_rx_param.total_pri = pulses_per_dwell * num_cpi_files;
                ...
                if cfg.run.do_process
                    ...
                    % 波位 RD 文件在 beam_XXX 子目录下，直接查找
                    beam_result_dir = fullfile(batch_result_dir, sprintf('beam_%03d', beam_id));
                    rd_pattern = sprintf('RD_Proc_beam%03d_*.mat', beam_id);
                    out_file = load_latest_rd_result(data_dir, '', rd_pattern, raw_spec.batch_name);
                    if isempty(out_file) || ~exist(out_file, 'file')
                        % 回退：直接在 beam 子目录下查找
                        entries = dir(fullfile(beam_result_dir, rd_pattern));
                        if ~isempty(entries)
                            [~, idx] = max([entries.datenum]);
                            out_file = fullfile(entries(idx).folder, entries(idx).name);
                        else
                            error('run_batch_pipeline:MissingBeamRD', ...
                                '未找到波位 %d 的 RD 文件：%s', beam_id, beam_result_dir);
                        end
                    end
                end
                        ...
                        if has_angle && n_clu > 0
                            ...
                            if ~isempty(r_m)
                                ...
                                for ci = 1:n_clu
                                    ...
                                    if ~isempty(idx_ci)
                                        pwr_ci = pwr(sub2ind(size(pwr), det_r_idx(idx_ci), det_v_idx(idx_ci)));
                                        [~, best] = max(pwr_ci);
                                        pwr_dB(ci) = 10 * log10(pwr_ci(best) + eps);
                                    end
                                end
                                ...
                                all_raw_plots = [all_raw_plots; ...
                                    ...
                                    pwr_dB(:), repmat(int32(beam_id), n_pts, 1)]; %#ok<AGROW>
                            end
                        end
                    end
                end
            ...
            % ---- 全景绘图（如果启用）----
            if cfg.run.do_plot
                fprintf('  [步骤10] 多波位全景绘图\n');
                plot_beam_panorama(fused_plots, all_raw_plots, beam_schedule, batch_result_dir, cfg);
            end
        ...
        [~, preproc_state] = preprocess('init', raw_spec, parse_bundle.tx, rd_ctx, cfg.preprocess, lg);
        ...
        if cfg.run.do_process
            ...
            out_file = load_latest_rd_result(data_dir, result_dir_name, rd_pattern, raw_spec.batch_name);
        end
...
function rd_file = load_latest_rd_result(data_dir, result_dir_name, rd_pattern, batch_name)
    ...
    % 收集所有候选 RD 文件，按时间从新到旧排序
    all_candidates = [];
    for si = 1:numel(search_dirs)
        if ~exist(search_dirs{si}, 'dir')
            continue;
        end
        entries = dir(fullfile(search_dirs{si}, rd_pattern));
        if ~isempty(entries)
            all_candidates = [all_candidates; entries]; %#ok<AGROW>
        end
    end
    ...
    % 从新到旧依次校验，跳过不完整文件（缺少 total_blocks 等关键变量）
    [~, ts_order] = sort([all_candidates.datenum], 'descend');
    all_candidates = all_candidates(ts_order);
    rd_file = '';
    for ci = 1:numel(all_candidates)
        candidate = fullfile(all_candidates(ci).folder, all_candidates(ci).name);
        try
            vars = who('-file', candidate);
            if ismember('total_blocks', vars) && ismember('r_axis_full', vars) && ismember('v_axis_full', vars)
                rd_file = candidate;
                break;
            end
        catch
    ...
    if isempty(rd_file)
        error('run_batch_pipeline:MissingRDResult', ...
            '找到 RD_Proc_*.mat 但均不包含必需变量（total_blocks, r_axis_full, v_axis_full）：%s', data_dir);
    end
end
...
function plot_beam_panorama(fused_plots, all_raw_plots, beam_schedule, result_dir, cfg)
%PLOT_BEAM_PANORAMA 多波位全景可视化。
%
% 输入：
%   fused_plots   : K×5, [r(m), az(deg), el(deg), vr(m/s), time(s)]
%   all_raw_plots : N×7, 融合前的全部原始点迹
%   beam_schedule : 波位排布结构体
%   result_dir    : 输出目录
%   cfg           : 全局配置
% 作用：
%   生成全景 RD 散点图（距离-速度，颜色=方位角）和波位检测数柱状图。

if isempty(fused_plots)
    fprintf('[全景绘图] 无融合目标，跳过绘图。\n');
    return;
end
...
% ---- 子图 1：全景 RD 散点图（融合后）----
subplot(1, 2, 1);
scatter(fused_plots(:, 1), fused_plots(:, 4), 30, fused_plots(:, 2), 'filled');
colormap(gca, jet);
cbar = colorbar;
cbar.Label.String = 'Azimuth (deg)';
caxis([min(beam_schedule.az_grid), max(beam_schedule.az_grid)]);
xlabel('Range (m)');
ylabel('Radial Velocity (m/s)');
title(sprintf('Fused Targets (N=%d)', size(fused_plots, 1)));
grid on;
axis tight;

% ---- 子图 2：各波位检测数柱状图 ----
subplot(1, 2, 2);
beam_counts = zeros(beam_schedule.num_beams, 1);
for b = 1:beam_schedule.num_beams
    beam_counts(b) = sum(all_raw_plots(:, 7) == b);
end
bar(1:beam_schedule.num_beams, beam_counts, 'FaceColor', [0.2 0.4 0.7]);
xlabel('Beam ID');
ylabel('Detection Count');
title('Detections per Beam');
grid on;

sgtitle('Multi-Beam Panoramic View');

% 保存
ts = char(datetime('now', 'Format', 'yyyyMMdd_HHmmss'));
png_file = fullfile(result_dir, sprintf('Panorama_%s.png', ts));
saveas(fig, png_file);
close(fig);
fprintf('[全景绘图] 已保存：%s\n', png_file);
end
