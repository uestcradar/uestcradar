...
cfg.beam.beam_file = '波位格式_128同向.txt';     % 波位排布文件路径（相对于工程根目录）
...
az_min = min(beam_schedule.az_grid);
az_max = max(beam_schedule.az_grid);
if az_max - az_min < 1e-6
    az_max = az_min + 5;  % 所有波位指向相同时，给一个默认色轴范围
end
caxis([az_min, az_max]);
xlabel('Range (m)');
