...
拓扑自动保存在当前浏览器、当前访问地址的 localStorage 中，包含节点顺序、Worker 镜像、RDMA 端口及 Ring 参数。刷新会恢复配置和仍有效的登录会话；会话过期时需要重新登录，拓扑仍保留，登录后会重新登记缺失节点并探查。离线节点的已选值会保留并显示“待探查确认”。刷新不会自动部署。
