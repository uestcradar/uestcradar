...
func TestBuildPlanDerivesRingCapacityWithoutWorkerArguments(t *testing.T) {
	...
	request := PlanRequest{Chain: []ChainEntry{{IP: "10.0.0.1", RDMADevice: "hns_1:1", WorkerImage: worker.Reference}, {IP: "10.0.0.2", RDMADevice: "hns_1:1", WorkerImage: worker.Reference}}, SlotCount: 8, MaxPayloadBytes: 128 << 20}
	plan, err := BuildPlan(request, nodes, "10.0.0.99", time.Now())
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(plan.Nodes[0].env, "SLOT_COUNT=8") || !strings.Contains(plan.Nodes[0].env, "MAX_PAYLOAD_BYTES=134217728") || !strings.Contains(plan.Nodes[0].env, "SIDECAR_SHM_SIZE=3g") {
		t.Fatalf("selected RingBuffer configuration missing:\n%s", plan.Nodes[0].env)
	}
	if strings.Contains(plan.Nodes[0].env, "CASCADE_PAYLOAD") || strings.Contains(plan.Nodes[0].compose, "payload-bytes") {
		t.Fatal("orchestration must not configure Worker payload arguments")
	}
	request.SHMSize = "2g"
	if _, err := BuildPlan(request, nodes, "10.0.0.99", time.Now()); err == nil || !strings.Contains(err.Error(), "must be 3g") {
		t.Fatalf("expected derived shm_size rejection, got %v", err)
	}
}
